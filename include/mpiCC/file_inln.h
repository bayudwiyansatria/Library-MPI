...
{
  MPI_Request req;
  (void) MPI_File_iread(mpi_file, buf, count, datatype,
                        (MPIO_Request *)&req);
  return req;
}
...
{
  MPI_Request req;
  (void) MPI_File_iread_at(mpi_file, offset, buf, count, datatype,
                           (MPIO_Request *)&req);
  return req;
}
...
{
  MPI_Request req;
  (void) MPI_File_iread_shared(mpi_file, buf, count, datatype,
                                (MPIO_Request *)&req);
  return req;
}
...
{
  MPI_Request req;
  (void) MPI_File_iwrite(mpi_file, (void*) buf, count, datatype,
                          (MPIO_Request *)&req);
  return req;
}
...
{
  MPI_Request req;
  (void) MPI_File_iwrite_shared(mpi_file, (void*) buf, count, datatype,
                                 (MPIO_Request *)&req);
  return req;
}
...
{
  (void) MPI_File_read(mpi_file, buf, count, datatype,
                        &status.mpi_status);
}
...
_REAL_MPI_::File::Read_all(void* buf, int count,
			   ...
			   _REAL_MPI_::Status& status)
{
  (void) MPI_File_read_all(mpi_file, buf, count, datatype,
                           &status.mpi_status);
}
...
{
  (void) MPI_File_read_all_end(mpi_file, buf,
                               &status.mpi_status);
}
...
{
  (void) MPI_File_read_at_all_end(mpi_file, buf,
                                  &status.mpi_status);
}
...
{
  (void) MPI_File_read_ordered_end(mpi_file, buf,
                                   &status.mpi_status);
}
...
{
  (void) MPI_File_write_all_end(mpi_file, (void*) buf,
                                &status.mpi_status);
}
...
{
  (void) MPI_File_write_at_all_end(mpi_file, (void*) buf,
                                   &status.mpi_status);
}
...
_REAL_MPI_::File::Write_ordered_end(const void* buf,
				    _REAL_MPI_::Status& status)
{
  (void) MPI_File_write_ordered_end(mpi_file, (void*) buf,
                                    &status.mpi_status);
}
...
inline void
_REAL_MPI_::File::Call_errhandler(int errorcode) const
{
  (void) MPI_File_call_errhandler(mpi_file, errorcode);
}

#ifdef HPMPI_SUPPORT_CREATEERR
inline _REAL_MPI_::Errhandler
_REAL_MPI_::File::Create_errhandler(
              _REAL_MPI_::File::Errhander_fn* function)
{
  MPI_Errhandler errhandler;
  // $%%@#%# AIX/POE 2.3.0.0 makes us put in this cast here
  (void)MPI_File_errhandler_create(
                  (MPI_Handler_function*) errhandler_intercept,
			      &errhandler);
  _REAL_MPI_::Errhandler temp(errhandler);
  temp.handler_fn = (void(*)(_REAL_MPI_::File&, int*, ...))function;
  return temp;
}
#endif
