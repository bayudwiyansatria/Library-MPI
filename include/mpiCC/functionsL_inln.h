...
inline void
_REAL_MPI_::Attach_bufferL(void* buffer, _REAL_MPI_::Aint size)
{
  (void)MPI_Buffer_attachL(buffer, size);
}

inline _REAL_MPI_::Aint
_REAL_MPI_::Detach_bufferL(void*& buffer)
{
  _REAL_MPI_::Aint size;
  (void)MPI_Buffer_detachL(&buffer, &size);
  return size;
}
